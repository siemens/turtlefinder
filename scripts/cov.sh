...
go test -exec sudo -cover -v -p=1 -count=1 -race -tags=matchers ./... -args -test.gocoverdir="$GOCOVERTMPDIR"
go test -cover -v -p=1 -count=1 -race -tags=matchers ./... -args -test.gocoverdir="$GOCOVERTMPDIR"
